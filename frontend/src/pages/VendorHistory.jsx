...
export default function VendorHistory() {
  ...
  if (loading) return (
    <div className="loading-screen">
      ...
      <p style={{ marginTop: '1rem', fontWeight: '700', color: 'var(--text-primary)' }}>Crunching your numbers...</p>
    </div>
  );
  ...
  return (
    <div className="page-wide" style={{ padding: '2.5rem' }}>
      ...
      {activeTab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: '1.5rem' }}>
                  ...
                  return (
                    <div key={status}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
                        ...
                        <span style={{ fontWeight: '800', color: 'var(--text-primary)', fontSize: '0.9rem' }}>{count} ({pct}%)</span>
                      </div>
          ...
          {sortedItems.length > 0 && (
            <div className="card" style={{ border: 'none', gridColumn: 'span 1' }}>
              <h3 style={{ fontWeight: 900, color: 'var(--text-primary)', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                Item Performance
              </h3>
                  ...
                  <div key={i} style={{ width: '100%' }}>
                    ...
                    <div style={{ height: '10px', background: 'var(--bg-secondary)', borderRadius: '5px', overflow: 'hidden' }}>
                      <div style={{
                        ...
                        background: i === 0 ? 'var(--text-primary)' : i === 1 ? '#e85d68' : i === 2 ? '#f0908a' : '#ddd',
                        borderRadius: '5px',
                        transition: 'width 1s ease-out'
                      }}></div>
                    </div>
      ...
      {activeTab === 'hourly' && (
        <div className="card" style={{ border: 'none' }}>
          <h3 style={{ fontWeight: 900, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Hourly Sales Heatmap</h3>
            ...
            {Array.from({ length: 24 }, (_, h) => {
              ...
              return (
                <div key={h} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                  <div style={{ fontSize: '0.65rem', fontWeight: '800', color: isPeak ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                    {d.orders > 0 ? d.orders : ''}
                  </div>
                  <div
          ...
          <div style={{ display: 'flex', gap: '3px', padding: '0.5rem 0.5rem 0', borderTop: '1px solid var(--border)' }}>
            {Array.from({ length: 24 }, (_, h) => (
              <div key={h} style={{
                ...
                color: h === peakHour ? 'var(--text-primary)' : 'var(--text-muted)',
              }}>
                {h % 3 === 0 ? `${h}` : ''}
              </div>
            ))}
          ...
          {peakHour !== null && (
            <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'rgba(154, 0, 2, 0.05)', borderRadius: '12px', display: 'flex', justifyContent: 'space-around', textAlign: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: '900', color: 'var(--text-primary)' }}>{peakTimeStr}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', fontWeight: '700', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Orders at Peak</div>
                <div style={{ fontSize: '1.25rem', fontWeight: '900', color: 'var(--text-primary)' }}>{hourlyData[peakHour]?.orders || 0}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', fontWeight: '700', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Revenue at Peak</div>
                <div style={{ fontSize: '1.25rem', fontWeight: '900', color: 'var(--text-primary)' }}>₹{(hourlyData[peakHour]?.revenue || 0).toLocaleString()}</div>
              </div>
            </div>
          )}
        </div>
      ...
      {activeTab === 'daily' && (
        <div className="card" style={{ border: 'none' }}>
          <h3 style={{ fontWeight: 900, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Daily Revenue Trend</h3>
      ...
      {activeTab === 'orders' && (
        <>
          {data.orders.length === 0 ? (
            ...
            <div className="table-wrapper" style={{ border: 'none', background: 'white' }}>
              <table>
                <thead>
                ...
                <tbody>
                  {data.orders.map((order) => (
                    <tr key={order._id}>
                      <td style={{ fontWeight: 800, color: 'var(--text-primary)' }}>#{order._id.slice(-6).toUpperCase()}</td>
                      <td><span className="badge" style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}>{order.eventCode}</span></td>
                      <td style={{ maxWidth: '200px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {order.items.map((it) => `${it.quantity}× ${it.name}`).join(', ')}
                      </td>
                      <td style={{ fontWeight: 900, color: 'var(--text-primary)' }}>₹{order.totalAmount}</td>
                      <td>
